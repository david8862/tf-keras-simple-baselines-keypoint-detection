...
void heatmap_postprocess(const Tensor* heatmap, std::vector<t_prediction> &prediction_list, float heat_threshold)
{
...
void adjust_scale(std::vector<t_prediction> &prediction_list, int image_width, int image_height, int input_width, int input_height)
{
    ...
    for(auto &prediction : prediction_list) {
        prediction.x = prediction.x * scale_width * OUTPUT_STRIDE;
        prediction.y = prediction.y * scale_height * OUTPUT_STRIDE;
    }
...
void RunInference(Settings* s) {
    ...
    for(auto output : outputs) {
        ...
        MNN_ASSERT((input_width/output_width == OUTPUT_STRIDE) && (input_height/output_height == OUTPUT_STRIDE));
    ...
    for (int i = 0; i < num_outputs; ++i) {
        ...
        heatmap_postprocess(heatmap, prediction_list, heat_threshold);
    }

    gettimeofday(&stop_time, nullptr);
    MNN_PRINT("heatmap_postprocess time: %lf ms\n", (get_us(stop_time) - get_us(start_time)) / 1000);
