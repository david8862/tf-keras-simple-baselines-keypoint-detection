...
def main():
    ...
    data_generator = keypoints_dataset(args.dataset_path, batch_size=1, class_names=class_names, input_shape=model_input_shape, is_train=True, with_meta=True)
    ...
    for i, (image_data, gt_heatmap, metainfo) in enumerate(data_generator):
        ...
        for j, keypoint in enumerate(gt_keypoints):
            gt_keypoints_dict[class_names[j]] = (keypoint[0]*OUTPUT_STRIDE, keypoint[1]*OUTPUT_STRIDE, 1.0)
