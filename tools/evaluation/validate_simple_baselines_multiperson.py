...
def predict_simple_baselines_model(model, image, model_input_shape, loop_count):
    # prepare input image
...
def predict_simple_baselines_model_tflite(interpreter, image, loop_count):
...
def predict_simple_baselines_model_mnn(interpreter, session, image, class_names, loop_count):
...
def predict_simple_baselines_model_onnx(model, image, class_names, loop_count):
    input_tensors = []
...
def predict_simple_baselines_model_pb(model, image, model_input_shape, loop_count):
    ...
    for image_file in image_files:
        ...
        for box, score in zip(person_boxes, person_scores):
            ...
            if args.model_path.endswith('.tflite'):
                keypoints = predict_simple_baselines_model_tflite(model, person_image, args.loop_count)
            # support of MNN model
            elif args.model_path.endswith('.mnn'):
                keypoints = predict_simple_baselines_model_mnn(model, session, person_image, class_names, args.loop_count)
            ## support of TF 1.x frozen pb model
            elif args.model_path.endswith('.pb'):
                keypoints = predict_simple_baselines_model_pb(model, person_image, model_input_shape, args.loop_count)
            # support of ONNX model
            elif args.model_path.endswith('.onnx'):
                keypoints = predict_simple_baselines_model_onnx(model, person_image, class_names, args.loop_count)
            ## normal keras h5 model
            elif args.model_path.endswith('.h5'):
                keypoints = predict_simple_baselines_model(model, person_image, model_input_shape, args.loop_count)
            else:
                raise ValueError('invalid model file')
            ...
            for i, keypoint in enumerate(keypoints):
                keypoints_dict[class_names[i]] = (keypoint[0] * scale[0] * OUTPUT_STRIDE + xmin, keypoint[1] * scale[1] * OUTPUT_STRIDE + ymin, keypoint[2])
