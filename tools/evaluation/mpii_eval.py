...
from eval import model_predict_keras, model_predict_tflite, model_predict_pb, model_predict_onnx, model_predict_mnn, revert_pred_keypoints, load_eval_model, draw_plot_func
...
def mpii_eval(model, model_format, eval_dataset, class_names, model_input_shape, score_threshold, conf_threshold):
    ...
    for image_data, gt_heatmap, metainfo in eval_dataset:
        ...
        # support of MNN model
            ...
            heatmap = model_predict_mnn(model, session, image_data)
        # support of TF 1.x frozen pb model
        elif model_format == 'PB':
            heatmap = model_predict_pb(model, image_data)
        # support of ONNX model
        elif model_format == 'ONNX':
            heatmap = model_predict_onnx(model, image_data)
